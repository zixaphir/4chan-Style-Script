  ...
  */
  ...
  $ = (function() {
    ...
    $.prototype.hasClass = function(className) {
      ...
      return (new RegExp("\\b" + className + "\\b")).test(this.elems[0].className);
    };
    ...
    options: {
      ...
      show: function() {
        ...
        if ($("#overlay").exists()) {
          ...
          optionsHTML = ["<ul id=toNav>", "<li><label class=selected for=tcbMain>Main</label></li>", "<li><label for=tcbThemes>Themes</label></li>", "<li><label for=tcbMascots>Mascots</label></li>", "</ul><div id=toWrapper><input type=radio name=toTab id=tcbMain hidden checked><div id=tMain>", "<p><a class=trbtn name=loadSysFonts title='Reqiures flash'>" + (SS.fontList ? 'System Fonts Loaded!' : 'Load System Fonts') + "</a></p>"];
      ...
      createThemesTab: function(tOptions) {
        var handler, i, max, p, themes;
        themes = $("#tThemes", tOptions).html("");
        p = $("<p style='bottom:8px!important'>");
        handler = function() {
          var div, err, file, index, reader, theme;
          file = this.files[0];
        ...
        p.append($("<div id=selectImage>").append($("<input type=file riced=true>").bind("change", handler)).append($("<span class=trbtn>Import")));
      ...
      save: function() {
        var div, links, mascots, selMascots, selTheme, themes;
        div = $("#themeoptions");
        themes = [];
        mascots = [];
        links = [];
        ...
        $("#themeoptions #tThemes>div").each(function(index) {
          ...
          if (!SS.conf["Themes"][oldIndex]["default"]) {
            return themes.push(SS.conf["Themes"][oldIndex]);
          }
        });
        selTheme = (selTheme = $("#themeoptions #tThemes>div.selected")).exists() ? parseInt(selTheme.attr("id").substr(5)) : 0;
        ...
        $("#themeoptions #tMascot>div").each(function(index) {
          ...
          return mascots.push(SS.conf["Mascots"][oldIndex](unlessSS.conf["Mascots"][oldIndex]["default"]));
        });
      ...
      showTheme: function(tIndex) {
        var bEdit, check, dName, div, innerHTML, name, tEdit, themeA, themePX, themePY, themeR, _i, _len, _ref, _ref1;
        ...
        return $(d.body).append($("<div id=overlay2>").append(div));
      },
    ...
    Theme: (function() {
      ...
      _Class.prototype.preview = function() {
        ...
        $(div).bind("click", function() {
          var theme;
          theme = $(this);
          if (theme.hasClass("selected")) {
            return;
          }
          theme.parent().children(".selected").removeClass("selected");
          return theme.addClass("selected");
        });
        $("a[title=Delete]", div).bind("click", function(e) {
          e.stopPropagation();
