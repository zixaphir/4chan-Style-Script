  ...
  */
  ...
  SS = {
    ...
    options: {
      ...
      show: function() {
        var bindNavLinks, cFonts, des, id, key, link, links, name, option, optionsHTML, opts, overlay, pVal, tOptions, text, val, value, _i, _j, _len, _len1, _ref, _results;
        if ($("#overlay").exists()) {
          SS.options.close();
        } else {
          ...
          bindNavLinks = function(el) {
            return $(".handle", el).bind("dragstart", function(e) {
          ...
          $("a[name=delLink]", el).bind("click", function() {
        ...
        for (key in defaultConfig) {
          ...
          if (defaultConfig[key][4] === true) {
            ...
            cFonts = [];
            Array.prototype.push.apply(optionsHTML, ["<span class=mOption title=\"" + des + "\"><span>E{keyE}</span>", "<select name='" + key + "'" + (defaultConfig[key][3] === true ? ' hasSub' : '') + ">"]);
            ...
            if (links != null) {
              for (_j = 0, _len1 = links.length; _j < _len1; _j++) {
                _ref = links[_j], link = _ref.link, text = _ref.text;
    ...
    showTheme: function(tIndex) {
      var RPA, bEdit, check, dName, div, innerHTML, name, overlay, tEdit, themeA, themePX, themePY, themeR, _i, _len, _ref;
      if (typeof tIndex === "number") {
        ...
        for (_i = 0, _len = themeInputs.length; _i < _len; _i++) {
          _ref = themeInputs[_i], name = _ref.name, dName = _ref.dName;
